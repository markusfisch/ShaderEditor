...
public final class Highlighter {
    ...
    private static Highlight highlightGroup(TokenType type) {
        switch (type) {
            case INVALID:
            case TOKEN_EOF:
    ...
    public static void highlight(@NonNull Spannable spannable, @NonNull String source) {
        int[] highlights = Native.highlight(source);
        if (highlights == null) return;
        for (int i = 0, length = highlights.length; i < length; i += 3) {
            TokenType type = TokenType.values()[highlights[i]];
            Highlight highlight = highlightGroup(type);
            int start = highlights[i + 1];
            int end = highlights[i + 2];
            spannable.setSpan(new ForegroundColorSpan(colors[highlight.ordinal()]), start, end, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
        }
    }
    ...
    private static class Native {
        public static native @Nullable int[] highlight(@NonNull String source);
    }
}
