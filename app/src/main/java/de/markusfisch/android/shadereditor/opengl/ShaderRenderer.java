...
public class ShaderRenderer implements GLSurfaceView.Renderer {
    ...
    private String indexTextureNames(String source) {
        ...
        if (!source.contains(SHADER_EDITOR)) {
            source = addPreprocessorDirective(source, SHADER_EDITOR, true);
        }
    ...
    private static String addPreprocessorDirective(String source,
                                                   String directive) {
        return addPreprocessorDirective(source, directive, false);
    }

    /**
     * @param source the source code without the directive
     * @param directive the directive to add
     * @param hidden    if {@code true}, insert a {@code #line} preprocesor directive, so that it does not count as a line
     * @return the modified source that includes the directive
     */
    private static String addPreprocessorDirective(String source,
                                                   String directive, boolean hidden) {
        // #version must always be the very first directive.
        boolean hasVersionDirective = source.trim().startsWith("#version");
        directive = hidden ? directive + "#line " + (hasVersionDirective ? 2 : 1) + "\n" : directive;
